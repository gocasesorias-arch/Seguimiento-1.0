  ...
  const renderMultiSelect = (label, placeholder, options, selected, onChange) => (
    <div>
      ...
      <details className="relative group">
        <summary className="w-full px-4 py-2 border border-slate-200 rounded-xl focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 cursor-pointer list-none flex items-center justify-between text-sm bg-white">
          <span className="truncate">
            {selected.length === 0 ? placeholder : `${selected.length} seleccionados`}
          </span>
          <span className="text-xs text-slate-400">▼</span>
        </summary>
        <div className="absolute left-0 right-0 mt-2 bg-white border border-slate-200 rounded-xl shadow-lg max-h-60 overflow-y-auto z-20">
          {options.map(opt => (
            <label
              key={opt.value}
              className="flex items-center gap-3 px-3 py-2 hover:bg-emerald-50 cursor-pointer text-sm"
            >
              <input
                type="checkbox"
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div>
          ...
          <select
            ...
            className="w-full px-4 py-2 border border-slate-200 rounded-xl focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 focus:outline-none transition-colors bg-white"
          >
